//===- RISCVDAGBuilder.cpp - Binary raiser utility llvm-mctoll -----------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the implementation of RISCVDAGBuilder class for use by
// llvm-mctoll.
//
//===----------------------------------------------------------------------===//
...
/// code properties.
void RISCVDAGBuilder::visitCC(const MachineInstr &mi, MachineSDNode *mnode) {
  ...
  // // RISCV32::CPSR register use index in MachineInstr.
  // int idx = mi.findRegisterUseOperandIdx(RISCV32::CPSR);
  // // Number of operands for MachineInstr.
  // int numOps = mi.getNumOperands();

  // // If the MachineInstr has RISCV32::CPSR register, update the NodePropertyInfo
  // // properties.
  // if (idx != -1 && !mi.getOperand(idx).isImplicit()) {
  //   // MI with RISCV32::CPSR register.
  //   if (idx != numOps - 1) {
  //     if (mi.getOperand(idx + 1).isReg() &&
  //         mi.getOperand(idx + 1).getReg() == RISCV::CPSR) {
